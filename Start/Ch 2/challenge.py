...
class Stock(Asset):
    ...
    def get_description(self):
        return f"Ticker: {self.company_name} -- ${self.price}"

class Bond(Asset):
    def __init__(self, price, description, duration, byield):
        super().__init__(price)
    ...
    def get_description(self):
        return f"{self.description}: {self.duration}yr : ${self.price} : {self.byield}%"
